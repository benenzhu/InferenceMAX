...
def generate_test_config(args, all_config_data):
    """Generate a single test configuration by config key.

    Looks up the config by key and returns it directly without any sweep logic.
    """
    config_key = args.config_key

    if config_key not in all_config_data:
        available_keys = ', '.join(sorted(all_config_data.keys()))
        raise ValueError(
            f"Config key '{config_key}' not found. Available keys: {available_keys}")

    val = all_config_data[config_key]
    is_multinode = val.get(Fields.MULTINODE.value, False)
    ...
    for seq_config in val[Fields.SEQ_LEN_CONFIGS.value]:
        isl = seq_config[Fields.ISL.value]
        osl = seq_config[Fields.OSL.value]
        bmk_space = seq_config[Fields.SEARCH_SPACE.value]
        ...
        for bmk in bmk_space:
            ...
            if is_multinode:
                ...
                # Get concurrency values directly from conc-list
                conc_list = bmk.get(Fields.CONC_LIST.value)
                if conc_list:
                    conc_values = conc_list
                else:
                    # Fallback to range if conc-list not specified
                    conc_start = bmk[Fields.CONC_START.value]
                    conc_end = bmk[Fields.CONC_END.value]
                    conc_values = []
                    conc = conc_start
...
def main():
    ...
    test_config_by_key_parser = subparsers.add_parser(
        'test-config',
        parents=[parent_parser],
        add_help=False,
        help='Generate configuration for a specific config key. Outputs the full config without any filtering or test mode logic.'
    )
    test_config_by_key_parser.add_argument(
        '--config-key',
        required=True,
        help='The exact config key to look up (e.g., dsr1-fp4-gb200-dynamo-trt)'
    )
    test_config_by_key_parser.add_argument(
        '-h', '--help',
        action='help',
